...
- **Rate-limit-gated routes after audit**: 50 distinct limiter mounts (was 16 before audit; latest add: `strictLimit` on `/api/process-content`)
- **Body-validated routes after audit**: **190** routes covered by per-route `validateBody(...)` or inline `validateRequest()` Zod schemas (was ~14). Every authenticated mutation now has a per-route Zod checkpoint — high-risk routes use field-level schemas, generic routes default to `emptyBodySchema` (passthrough object), applied via the codemod `scripts/inject-default-body-validation.ts`.
- **Baseline body validation**: 100% coverage — every POST/PUT/PATCH ALSO passes through the new global `requireJsonObjectBody` middleware (`server/middleware/security.ts`, wired in `server/index.ts`) which rejects bare arrays, primitives, buffers, and root-level prototype-pollution keys (`__proto__`, `constructor`, `prototype`) with HTTP 400 before auth or handler runs. Verified via curl: `[1,2,3]` → 400, `{"__proto__":{...}}` → 400, `"hello"` → 400, `{}` → 401 (auth still enforced).
- **Secret logging removed**: stripped the OpenAI API key preview log line in `server/index.ts` (was printing `sk-…` prefix + suffix on every startup); only the key length is now logged for debug visibility.
...
- **Policy verification (latest run)**: ✅ **0 errors, 0 warnings**. All six policy rules (auth, admin, AI rate limit, financial rate-limit-AND-Zod, AI/admin Zod, all-mutation per-route Zod) are 100% green. Rule 6 was promoted from warn → error after the codemod brought per-route Zod coverage to 100% — any future regression that lands an authenticated mutation without a Zod schema now fails CI.
