          ...
          {market.tags && market.tags.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              {market.tags.slice(0, 3).map((tag, i) => (
                <span key={i} className="px-2 py-0.5 bg-slate-800/50 text-slate-400 text-xs rounded-full border border-slate-700/50">
                  #{tag}
                </span>
              ))}
            </div>
...
const StatCard = ({ label, value, icon: Icon, color }: { label: string; value: string; icon: any; color: string }) => (
  <motion.div
    ...
    className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border border-slate-700/50 rounded-lg p-4 backdrop-blur-sm"
  >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500/20 to-cyan-500/20 border border-purple-500/30 rounded-full mb-6">
            ...
            <span className="text-sm font-medium text-purple-300">Prediction Markets</span>
          </div>
