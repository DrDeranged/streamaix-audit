...
import {
  useWebSocketPrices,
  type PriceUpdate,
} from "@/hooks/use-websocket-prices";
import {
...
function CollapsibleSection({
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <Surface className="bg-ink-raised overflow-hidden">
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className={cn("p-2 rounded-xl bg-ink-surface")}>
    ...
    <div className="relative w-32 h-32">
      <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
        {entries.map(([type, percent]) => {
          ...
          const color =
            assetTypeChartColors[type] || assetTypeChartColors.other;
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < points; i++) {
      ...
      const value =
        100 +
        change * progress +
        (Math.random() - 0.5) * Math.abs(change) * 0.3;
      result.push({ value });
    }
    return result;
  }, [priceChange7d]);
...
const NetWorthChart = memo(function NetWorthChart({
  portfolioId,
}: {
  portfolioId: string;
}) {
  ...
  if (chartData.length === 0) {
    return (
      <div className="h-48 flex items-center justify-center border border-dashed border-ink-edge rounded-xl">
        <div className="text-center">
          <LineChart className="w-8 h-8 mx-auto text-muted mb-2" />
          <p className="text-sm text-secondary">Not enough data for chart</p>
        </div>
      </div>
    );
  }
    ...
    <div data-testid="net-worth-chart">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-3 h-0.5 bg-accent-core" />
            <span className="text-xs text-secondary">Value</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-0.5 bg-ink-raised-foreground" />
            <span className="text-xs text-secondary">Cost Basis</span>
          </div>
        </div>
...
function WatchlistItemRow({
  ...
  const colorGradient =
    assetTypeColors[item.assetType] || assetTypeColors.other;
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className="flex items-center justify-between p-3 bg-ink-surface surface-interactive rounded-xl group"
      data-testid={`watchlist-item-${item.symbol}`}
    >
      <div className="flex items-center gap-3">
        <div className={cn("p-2 rounded-xl bg-ink-raised", colorGradient)}>
      ...
      <div className="flex items-center gap-3">
        <div className="text-right">
          <p className="font-medium text-primary text-sm">
            $
            {(item.currentPrice || item.addedPrice).toLocaleString(undefined, {
          ...
          >
            ...
            <span>
              {isPositive ? "+" : ""}
              {priceChange.toFixed(1)}% since added
            </span>
          </div>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button
            ...
            size="sm"
            onClick={() => onBuy(item)}
            className="h-7 px-2 text-gain hover:bg-gain/20"
          ...
          <Button
            ...
            className="h-7 px-2 text-secondary hover:text-loss hover:bg-loss/20"
...
function AddToWatchlistDialog({ onSuccess }: { onSuccess: () => void }) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const [cryptoRes, stockRes] = await Promise.all([
          fetch(`/api/crypto/search?q=${encodeURIComponent(searchQuery)}`)
            .then((r) => r.json())
            .catch(() => ({ coins: [] })),
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Asset is now being tracked",
      });
      setOpen(false);
      setSearchQuery("");
      setCryptoResults([]);
  ...
  const handleSelect = async (result: any, type: "crypto" | "stock") => {
    ...
    try {
      if (type === "crypto") {
        const res = await fetch(`/api/crypto/${result.id}/price`).then((r) =>
          r.json()
        );
        price = res.price || 0;
      } else {
        const res = await fetch(`/api/stocks/${result.symbol}/quote`).then(
          (r) => r.json()
        );
        price = res.price || 0;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-ink-raised sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-primary">Add to Watchlist</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="relative">
            ...
            />
            {isSearching && (
              <RefreshCw className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted animate-spin" />
            )}
          </div>
          ...
          {hasResults && (
            <div className="border border-ink-edge rounded-xl overflow-hidden max-h-[300px] overflow-y-auto">
              {stockResults.length > 0 && (
                <>
                  <div className="px-3 py-2 bg-ink-raised text-xs text-secondary font-medium flex items-center gap-2 border-b border-ink-edge">
                  ...
                  {stockResults.slice(0, 5).map((result) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-accent/40 text-left border-b border-ink-edge last:border-b-0 transition-colors disabled:opacity-50"
                    >
                      <div className="w-9 h-9 rounded-xl bg-accent-core/10 flex items-center justify-center">
                        <Building2 className="w-4 h-4 text-accent-bright" />
                      </div>
...
function BotTradingTab() {
  ...
  if (stakesLoading) {
    return (
      <div className="py-8 text-center">
        <RefreshCw className="w-6 h-6 text-accent-bright animate-spin mx-auto mb-2" />
        <p className="text-secondary text-sm">Loading bot stakes...</p>
      </div>
    );
  }
  ...
  const totalPnL = stakes.reduce(
    (sum: number, s: any) =>
      sum + Number(s.currentValue || s.amount || 0) - Number(s.amount || 0),
    0
  );
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-ink-surface rounded-xl p-3">
          <p className="text-[11px] text-secondary mb-1">Total Staked</p>
          <p className="text-base font-bold text-primary numeric">
            {totalStaked.toLocaleString()}{" "}
            <span className="text-xs text-secondary">STREAM</span>
          </p>
        </div>
        <div className="bg-ink-surface rounded-xl p-3">
          <p className="text-[11px] text-secondary mb-1">Active Bots</p>
          <p className="text-base font-bold text-accent-bright">
            {stakes.filter((s: any) => s.status === "active").length}
          </p>
        </div>
        <div className="bg-ink-surface rounded-xl p-3">
          <p className="text-[11px] text-secondary mb-1">Est. P&L</p>
          <p
      ...
      {stakes.length === 0 ? (
        <div className="py-6 text-center">
          <Bot className="w-10 h-10 text-accent-bright/50 mx-auto mb-3" />
          <h3 className="text-sm font-semibold text-primary mb-1">
            No Bot Stakes Yet
          </h3>
          <p className="text-xs text-secondary mb-4">
            Stake STREAM points on AI trading bots to earn simulated returns.
          </p>
          <Button
            size="sm"
        ...
        <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
          {stakes.map((stake: any) => {
            const pnl =
              Number(stake.currentValue || stake.amount || 0) -
              Number(stake.amount || 0);
            const pnlPct =
              Number(stake.amount) > 0 ? (pnl / Number(stake.amount)) * 100 : 0;
            return (
              <div
                key={stake.id}
                className="flex items-center justify-between p-3 bg-ink-surface surface-interactive rounded-xl hover:border-neon-cyan/50 cursor-pointer"
                onClick={() => navigate("/bot-trading")}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-primary">
                      {stake.botName || "Trading Bot"}
                    </p>
                    <p className="text-[11px] text-secondary numeric">
                      {Number(stake.amount).toLocaleString()} STREAM staked
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p
                    className={`text-[11px] ${
                      pnlPct >= 0 ? "text-gain/70" : "text-loss/70"
                    }`}
                  >
                    {pnlPct >= 0 ? "+" : ""}
                    {pnlPct.toFixed(1)}%
          ...
          <Button
            variant="outline"
            size="sm"
            className="w-full mt-2 border-accent-core/30 text-accent-bright hover:bg-accent-core/10"
            onClick={() => navigate("/bot-trading")}
          >
            View All Bots
...
function WatchlistTab({
  ...
  portfolioId: string;
  onBuyFromWatchlist: (item: WatchlistItemType) => void;
}) {
  const { toast } = useToast();
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<{ items: WatchlistItemType[] }>({
    ...
    return (
      <div className="py-8">
        <div className="text-center max-w-sm mx-auto">
          <motion.div className="w-20 h-20 mx-auto mb-5 rounded-2xl bg-ink-raised    flex items-center justify-center">
            <BookMarked className="w-10 h-10 text-warn" />
          </motion.div>
          <h3 className="text-xl font-bold text-primary mb-2">
            Track Assets to Watch
          </h3>
          <p className="text-secondary text-sm mb-6">
            Add stocks and crypto to your watchlist to track their prices
            without buying them.
          </p>
...
const TopMovers = memo(function TopMovers({
  ...
  const gainers = useMemo(
    () => sortedByChange.filter((a) => (a.priceChange24h || 0) > 0).slice(0, 3),
    [sortedByChange]
  );
  ...
  if (assets.length === 0) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-sm text-secondary">Add assets to see top movers</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <h4 className="text-xs font-medium text-gain mb-2 flex items-center gap-1">
        ...
        <div className="space-y-2">
          {gainers.length > 0 ? (
            gainers.map((asset) => (
              <div
                key={asset.id}
                className="flex items-center justify-between p-2 bg-gain/5 border border-gain/20 rounded-xl"
              >
            ...
            <p className="text-xs text-muted py-2">No gainers today</p>
          )}
        </div>
      </div>
      <div>
        <h4 className="text-xs font-medium text-loss mb-2 flex items-center gap-1">
        ...
        <div className="space-y-2">
          {losers.length > 0 ? (
            losers.map((asset) => (
              <div
                key={asset.id}
                className="flex items-center justify-between p-2 bg-loss/5 border border-loss/20 rounded-xl"
              >
                <span className="text-sm font-medium text-primary">
                  {asset.symbol}
                </span>
            ...
            <p className="text-xs text-muted py-2">No losers today</p>
          )}
        </div>
      </div>
    </div>
...
const DIVIDEND_DATA: Record<string, Omit<DividendInfo, "symbol" | "name">> = {
  AAPL: {
    nextDividendDate: "2025-02-14",
    dividendAmount: 0.25,
    frequency: "quarterly",
    yield: 0.48,
  },
  MSFT: {
    nextDividendDate: "2025-03-13",
    dividendAmount: 0.83,
    frequency: "quarterly",
    yield: 0.72,
  },
  JNJ: {
    nextDividendDate: "2025-03-11",
    dividendAmount: 1.24,
    frequency: "quarterly",
    yield: 3.0,
  },
  KO: {
    nextDividendDate: "2025-04-01",
    dividendAmount: 0.485,
    frequency: "quarterly",
    yield: 2.85,
  },
  PG: {
    nextDividendDate: "2025-02-18",
  ...
  JPM: {
    nextDividendDate: "2025-04-05",
    dividendAmount: 1.25,
    frequency: "quarterly",
    yield: 2.1,
  },
  VZ: {
    nextDividendDate: "2025-02-03",
  ...
  VYM: {
    nextDividendDate: "2025-03-25",
    dividendAmount: 0.85,
    frequency: "quarterly",
    yield: 2.6,
  },
  SCHD: {
    nextDividendDate: "2025-03-27",
    dividendAmount: 0.63,
    frequency: "quarterly",
    yield: 3.4,
  },
  VIG: {
    nextDividendDate: "2025-03-26",
    dividendAmount: 0.78,
    frequency: "quarterly",
    yield: 1.7,
  },
...
const BenchmarkComparisonChart = memo(function BenchmarkComparisonChart({
  portfolioId,
  assets,
}: {
  portfolioId: string;
  ...
  const chartData = useMemo(() => {
    ...
    return filteredSnapshots.map((s, index) => {
      const portfolioReturn =
        baseValue > 0 ? ((s.totalValue - baseValue) / baseValue) * 100 : 0;
  ...
  if (chartData.length < 2 || assets.length === 0) {
    return (
      <div className="h-64 flex items-center justify-center border border-dashed border-ink-edge rounded-xl">
        <div className="text-center">
          <LineChart className="w-8 h-8 mx-auto text-muted mb-2" />
          <p className="text-sm text-secondary">
            Not enough history for benchmark comparison
          </p>
          <p className="text-xs text-muted mt-1">
            Add more assets and check back in a few days
          </p>
        </div>
      </div>
    );
    ...
    <div data-testid="benchmark-comparison-chart">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <div className="w-3 h-0.5 bg-warn" />
      ...
      <div
        className={cn(
          "mt-3 p-2 rounded-xl text-center text-xs",
      ...
      >
        {portfolioReturn >= spReturn
          ? `📈 Outperforming S&P 500 by ${(portfolioReturn - spReturn).toFixed(
              1
            )}%`
          : `📉 Underperforming S&P 500 by ${(
              spReturn - portfolioReturn
            ).toFixed(1)}%`}
      </div>
    </div>
  );
...
function TransactionHistoryDialog({
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case "buy":
        return "text-gain bg-gain/20";
      case "sell":
        return "text-loss bg-loss/20";
      case "dividend":
        return "text-warn bg-warn/20";
      case "transfer":
        return "text-accent-bright bg-accent-core/20";
      default:
        return "text-secondary bg-ink-raised/20";
    }
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-ink-raised sm:max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-primary flex items-center gap-2">
            <History className="w-5 h-5 text-accent-bright" />
            Transaction History
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          <Select value={filterType} onValueChange={setFilterType}>
            ...
            <SelectContent className="bg-ink-raised">
          ...
          {isLoading ? (
            <div className="space-y-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="p-3 bg-ink-surface rounded-xl animate-pulse"
                >
                  <div className="h-4 bg-ink-raised rounded w-3/4 mb-2"></div>
                  <div className="h-3 bg-ink-raised rounded w-1/2"></div>
                </div>
              ))}
            </div>
          ) : filteredTransactions.length === 0 ? (
            <div className="text-center py-12">
              <Receipt className="w-12 h-12 mx-auto text-muted mb-3" />
              <p className="text-secondary">No transactions found</p>
              <p className="text-xs text-muted mt-1">
                Transactions will appear here when you add trades
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {paginatedTransactions.map((tx) => (
                <div
                  key={tx.id}
                  className="p-3 bg-ink-surface surface-interactive rounded-xl"
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <Badge
                        className={cn(
                          "text-xs uppercase",
                          getTypeColor(tx.transactionType)
                        )}
                      >
                        {tx.transactionType}
                      </Badge>
                      ...
                      <span className="text-xs text-muted">{tx.name}</span>
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      <span className="text-xs text-muted block">Fees</span>
                  ...
                  {tx.notes && (
                    <p className="text-xs text-muted mt-2 italic">{tx.notes}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-ink-edge">
            <span className="text-xs text-muted">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="border-ink-edge text-secondary"
...
function DividendCalendar({ assets }: { assets: PortfolioAsset[] }) {
  const dividendEligibleAssets = assets.filter(
    (a) =>
      ["stock", "etf"].includes(a.assetType) &&
      DIVIDEND_DATA[a.symbol.toUpperCase()]
  );

  const upcomingDividends = useMemo(() => {
    const now = new Date();
    const dividends: Array<
      DividendInfo & { estimatedPayment: number; sharesOwned: number }
    > = [];
    ...
    dividendEligibleAssets.forEach((asset) => {
      ...
      if (divData) {
        ...
        while (nextDate < now) {
          const daysToAdd =
            divData.frequency === "quarterly"
              ? 91
              : divData.frequency === "monthly"
              ? 30
              : 365;
          nextDate = new Date(
            nextDate.getTime() + daysToAdd * 24 * 60 * 60 * 1000
          );
        }
    ...
    return dividends.sort(
      (a, b) =>
        new Date(a.nextDividendDate).getTime() -
        new Date(b.nextDividendDate).getTime()
    );
  ...
  if (dividendEligibleAssets.length === 0) {
    return (
      <div className="text-center py-6">
        <Calendar className="w-8 h-8 mx-auto text-muted mb-2" />
        <p className="text-sm text-secondary">
          No dividend-paying stocks in portfolio
        </p>
        <p className="text-xs text-muted mt-1">
          Add stocks like AAPL, MSFT, or dividend ETFs
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4" data-testid="dividend-calendar">
      <div className="p-3 bg-ink-raised   border border-gain/20 rounded-xl">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-secondary">
            Est. Annual Dividend Income
          </span>
          <Badge className="text-[10px] bg-gain/20 text-gain">Projected</Badge>
        </div>
        <p className="text-xl font-bold text-gain">
          $
          {annualDividendIncome.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}
        </p>
        <p className="text-[10px] text-muted mt-1">
          ${(annualDividendIncome / 12).toFixed(2)}/month avg
        </p>
      </div>

      <div>
        <h4 className="text-xs font-medium text-secondary mb-3 flex items-center gap-2">
        ...
        <div className="space-y-2">
          {nextThreeDividends.map((div, idx) => {
            const daysUntil = Math.ceil(
              (new Date(div.nextDividendDate).getTime() - Date.now()) /
                (1000 * 60 * 60 * 24)
            );
            return (
              <div
                key={div.symbol}
                className="p-3 bg-ink-surface surface-interactive rounded-xl hover:border-neon-purple/50"
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-secondary bg-ink-raised px-1.5 py-0.5 rounded">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-xs text-muted">
                    {new Date(div.nextDividendDate).toLocaleDateString(
                      "en-US",
                ...
                <p className="text-[10px] text-muted mt-1">
                  {div.sharesOwned.toLocaleString()} shares × $
                  {div.dividendAmount.toFixed(4)}/share
                </p>
              </div>
            );
          })}
...
function PerformanceSummary({
  ...
  const periodReturns = useMemo(() => {
    ...
    const getReturnForDays = (days: number): number => {
      ...
      const olderSnapshots = snapshots.filter(
        (s) => new Date(s.snapshotDate) <= targetDate
      );
      const closestSnapshot =
        olderSnapshots.length > 0 ? olderSnapshots[0] : null;
      ...
      if (!closestSnapshot) {
        ...
        if (
          oldestSnapshot &&
          new Date(oldestSnapshot.snapshotDate) > targetDate
        ) {
        ...
        return oldestSnapshot && oldestSnapshot.totalValue > 0
          ? ((currentValue - oldestSnapshot.totalValue) /
              oldestSnapshot.totalValue) *
              100
          : 0;
      }
      ...
      return (
        ((currentValue - closestSnapshot.totalValue) /
          closestSnapshot.totalValue) *
        100
      );
    };
  ...
  if (assets.length === 0) {
    return (
      <div className="h-32 flex items-center justify-center border border-dashed border-ink-edge rounded-xl">
        <div className="text-center">
          <History className="w-6 h-6 text-muted mx-auto mb-2" />
          <p className="text-xs text-muted">Add assets to see performance</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {periodReturns.map((period) => (
          <div
            key={period.label}
            className="p-3 bg-ink-surface rounded-xl text-center"
          >
            ...
            <span
              className={cn(
                "text-sm font-bold",
                period.value >= 0 ? "text-gain" : "text-loss"
              )}
            >
              {period.value >= 0 ? "+" : ""}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-ink-raised   rounded-xl border border-gain/20">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-4 h-4 text-gain" />
            <span className="text-xs text-secondary">Best Performer</span>
          </div>
          {topGainer ? (
        ...
        <div className="p-3 bg-ink-raised   rounded-xl border border-loss/20">
          <div className="flex items-center gap-2 mb-2">
            <TrendingDown className="w-4 h-4 text-loss" />
            <span className="text-xs text-secondary">Worst Performer</span>
          </div>
          {topLoser && (topLoser.priceChange24h || 0) < 0 ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted">No losers</span>
            </div>
          )}
        </div>
      </div>
...
function GoalTracker({ currentValue }: { currentValue: number }) {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<{ success: boolean; goals: PortfolioGoal[] }>({
  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        {[1, 2].map((i) => (
          <div key={i} className="p-3 bg-ink-surface rounded-xl animate-pulse">
            <div className="h-4 bg-ink-raised rounded w-3/4 mb-2"></div>
            <div className="h-2 bg-ink-raised rounded w-full"></div>
          </div>
        ))}
      </div>
    );
    ...
    <div className="space-y-3">
      {goals.length === 0 ? (
        <div className="text-center py-4">
          <Target className="w-6 h-6 mx-auto text-muted mb-2" />
          <p className="text-xs text-muted">No goals set yet</p>
        </div>
      ) : (
        goals.map((goal) => {
          ...
          const daysLeft = goal.deadline
            ...
            : null;
          return (
            <div key={goal.id} className="p-3 bg-ink-surface rounded-xl group">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-primary">
                  {goal.name}
                </span>
              ...
              <div className="flex justify-between mt-2 text-[11px] sm:text-[10px] text-muted">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className="w-full text-xs text-accent-bright hover:text-accent-bright"
        ...
        <DialogContent className="bg-ink-raised sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-primary">Create New Goal</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div>
              <Label className="text-body text-sm">Goal Name *</Label>
              <Input
                value={newGoalName}
            ...
            <div>
              <Label className="text-body text-sm">Target Amount *</Label>
              <Input
                type="number"
                value={newGoalTarget}
            ...
            <div>
              <Label className="text-body text-sm">Deadline (Optional)</Label>
              <Input
                type="date"
            ...
            <div>
              <Label className="text-body text-sm">Goal Type</Label>
            ...
            <Button
              ...
              className="w-full bg-ink-raised  "
...
function CorrelationMatrix({
  ...
  if (assets.length < 2) {
    return (
      <div className="text-center py-4">
        <PieChart className="w-6 h-6 mx-auto text-muted mb-2" />
        <p className="text-xs text-muted">Add 2+ assets to see correlations</p>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="text-center py-4">
        <RefreshCw className="w-6 h-6 mx-auto text-muted mb-2 animate-spin" />
        <p className="text-xs text-muted">Loading correlations...</p>
      </div>
    );
  }
    ...
    <div className="overflow-x-auto -mx-2 px-2">
      <table className="w-full text-xs min-w-[280px]">
        <thead>
          <tr>
            ...
            {topAssets.map((a) => (
              <th
                key={a.symbol}
                className="p-1.5 text-secondary font-medium text-[11px]"
              >
                {a.symbol}
              </th>
            ))}
        ...
        <tbody>
          {topAssets.map((a) => (
            <tr key={a.symbol}>
              <td className="p-1.5 text-secondary font-medium text-[11px]">
                {a.symbol}
              </td>
              {topAssets.map((b) => {
                const corrValue =
                  correlations[a.symbol]?.[b.symbol] ??
                  (a.symbol === b.symbol ? 1 : 0.5);
                return (
      ...
      <div className="flex items-center justify-center gap-4 mt-3 text-[11px] sm:text-[10px] text-muted">
        <span className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-loss/60"></div> High
        </span>
        <span className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-ink-raised/30"></div> Low
        </span>
        <span className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-accent-core/40"></div> Negative
        </span>
      </div>
    </div>
  );
...
function IncomeTracker({ assets }: { assets: PortfolioAsset[] }) {
  ...
  const estimatedAnnualDividends = dividendAssets.reduce((sum, a) => {
    const yieldRate =
      a.assetType === "etf" ? 0.025 : a.assetType === "stock" ? 0.02 : 0.015;
  ...
  const estimatedStakingRewards = stakingAssets.reduce((sum, a) => {
    const stakingRate = ["ETH", "SOL", "ADA", "DOT"].includes(a.symbol)
      ? 0.05
      : 0.02;
  ...
  if (assets.length === 0) {
    return (
      <div className="text-center py-4">
        <DollarSign className="w-6 h-6 mx-auto text-muted mb-2" />
        <p className="text-xs text-muted">
          Add assets to estimate passive income
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="p-3 bg-ink-raised   border border-gain/20 rounded-xl">
        ...
        <p className="text-xl font-bold text-gain">
          $
          {totalPassiveIncome.toLocaleString(undefined, {
            maximumFractionDigits: 0,
          })}
        </p>
        <p className="text-[11px] sm:text-[10px] text-muted mt-1">
          ${(totalPassiveIncome / 12).toFixed(0)}/month
        </p>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div className="p-2 bg-ink-surface rounded-xl">
          <span className="text-[11px] sm:text-[10px] text-secondary">
            Dividends
          </span>
          <p className="text-sm font-medium text-primary">
            $
            {estimatedAnnualDividends.toLocaleString(undefined, {
              maximumFractionDigits: 0,
            })}
          </p>
        </div>
        <div className="p-2 bg-ink-surface rounded-xl">
          ...
          <p className="text-sm font-medium text-primary">
            $
            {estimatedStakingRewards.toLocaleString(undefined, {
              maximumFractionDigits: 0,
            })}
          </p>
        </div>
      </div>
      <div className="text-[11px] sm:text-[10px] text-muted text-center">
...
const NewsAggregator = memo(function NewsAggregator({
  assets,
}: {
  assets: PortfolioAsset[];
}) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery<{ success: boolean; news: NewsItem[] }>({
  ...
  if (assets.length === 0) {
    return (
      <div className="text-center py-4">
        <FileText className="w-6 h-6 mx-auto text-muted mb-2" />
        <p className="text-xs text-muted">Add assets to see relevant news</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-2">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="p-2.5 bg-ink-surface rounded-xl animate-pulse"
          >
            <div className="h-4 bg-ink-raised rounded w-3/4 mb-2"></div>
            <div className="h-3 bg-ink-raised rounded w-1/2"></div>
          </div>
  ...
  if (error || newsItems.length === 0) {
    return (
      <div className="text-center py-4">
        <FileText className="w-6 h-6 mx-auto text-muted mb-2" />
        <p className="text-xs text-muted">No relevant news found</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      {newsItems.slice(0, 4).map((item, i) => (
        <div
          key={i}
          className="p-2.5 bg-ink-surface rounded-xl hover:bg-accent/40 transition-colors cursor-pointer group"
        ...
        >
          <div className="flex items-start gap-2">
            ...
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs text-primary leading-snug line-clamp-2 group-hover:text-accent-bright transition-colors">
                {item.title}
              </p>
              <div className="flex items-center gap-2 mt-1">
                <Badge
                  variant="outline"
                  className="text-[11px] sm:text-[9px] px-1.5 py-0.5 text-secondary border-ink-edge"
                >
                  {item.symbol}
                </Badge>
                <span className="text-[11px] sm:text-[10px] text-muted">
...
const SectorBreakdownChart = memo(function SectorBreakdownChart({
  assets,
}: {
  assets: PortfolioAsset[];
}) {
  const sectorData = useMemo(() => {
    ...
    assets.forEach((asset) => {
      ...
      sectorTotals[sector] =
        (sectorTotals[sector] || 0) + (asset.currentValue || 0);
  ...
  if (assets.length === 0 || sectorData.length === 0) {
    return (
      <div className="text-center py-6">
        <PieChart className="w-8 h-8 mx-auto text-muted mb-2" />
        <p className="text-sm text-secondary">
          Add assets to see sector breakdown
        </p>
      </div>
    );
  }
    ...
    <div data-testid="sector-breakdown-chart">
      ...
      <div className="space-y-1.5 max-h-[180px] overflow-y-auto">
        {sectorData.slice(0, 8).map((item) => (
          <div
            key={item.sector}
            className="flex items-center justify-between py-1 px-2 rounded-md hover:bg-accent/40 transition-colors"
          >
            <div className="flex items-center gap-2">
              <div
              ...
              />
              <span className="text-xs text-body">{item.sector}</span>
            </div>
...
function ConnectedAccountsSection() {
  ...
  return (
    <div data-testid="connected-accounts-section">
      <div className="space-y-2">
        {demoConnectedAccounts.map((account) => {
          ...
          return (
            <div
              key={account.id}
              className="flex items-center justify-between p-3 bg-ink-surface rounded-xl"
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={cn(
                    "p-2 rounded-xl",
                    isConnected ? "bg-gain/10" : "bg-ink-raised/10"
                  )}
                >
                  <Icon
                    className={cn(
                      "w-4 h-4",
                      isConnected ? "text-gain" : "text-muted"
                    )}
                  />
                </div>
                <div>
      ...
      <Button
        variant="outline"
        className="w-full mt-3 border-accent-core/30 text-accent-bright hover:bg-accent-core/10 text-xs h-8"
        onClick={() =>
          toast({
            title: "Coming Soon",
            description: "Plaid integration for automatic account syncing",
          })
        }
...
function AIAdvisorChat({
  ...
  const sendMessage = async (question: string) => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        role: "assistant",
        content:
          "I'm having trouble connecting right now. Please try again later or contact support if the issue persists.",
      };
  ...
  return (
    <Surface
      className="bg-ink-raised    border-accent-core/30"
      data-testid="ai-advisor-chat"
    >
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger className="w-full p-4 flex items-center justify-between hover:bg-accent/40 transition-colors">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl bg-ink-raised  ">
          ...
          <div className="flex items-center gap-2">
            <Badge className="bg-ink-raised   border-accent-core/30 text-accent-bright text-[10px]">
        ...
        <CollapsibleContent>
          <div className="px-4 pb-4">
            <div className="h-[200px] overflow-y-auto mb-3 space-y-2 bg-ink-surface rounded-xl p-3">
              {messages.length === 0 && (
                <div className="text-center py-6">
                  <Brain className="w-8 h-8 mx-auto text-accent-bright mb-2" />
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {suggestedQuestions.map((q, i) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="text-[10px] h-6 px-2 border-ink-edge text-secondary hover:text-foreground hover:border-neon-purple/60"
              ...
              {isLoading && (
                <div className="bg-ink-raised text-secondary p-2 rounded-xl text-xs mr-8 flex items-center gap-2">
...
const AssetNewsFeed = memo(function AssetNewsFeed({
  assets,
}: {
  assets: PortfolioAsset[];
}) {
...
function AssetNewsSection({
  ...
  return (
    <Collapsible open={isExpanded} onOpenChange={onToggle}>
      <CollapsibleTrigger
        className="w-full p-2 bg-ink-surface rounded-xl flex items-center justify-between hover:bg-accent/40 transition-colors"
        data-testid={`news-toggle-${symbol}`}
      >
        <div className="flex items-center gap-2">
          <Badge
            variant="outline"
            className="text-[10px] px-1.5 py-0.5 text-secondary border-ink-edge"
          >
            {symbol}
          </Badge>
      ...
      <CollapsibleContent>
        <div className="mt-1 space-y-1 pl-2">
          {isLoading ? (
            <div className="p-2 text-[10px] text-muted flex items-center gap-2">
              <RefreshCw className="w-3 h-3 animate-spin" />
              Loading news...
            </div>
          ) : newsItems.length === 0 ? (
            <p className="p-2 text-[10px] text-muted">No recent news</p>
          ) : (
            <>
              {newsItems.slice(0, 3).map((item, i) => (
                <div
                  key={i}
                  className="p-2 bg-ink-surface surface-interactive rounded cursor-pointer"
                  onClick={() => item.url && window.open(item.url, "_blank")}
                >
              ...
              <Button
                variant="ghost"
                size="sm"
                className="w-full text-[10px] text-accent-bright hover:text-accent-bright h-6"
              >
...
function AddAssetDialog({
  ...
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<"search" | "manual" | "cash" | "retirement">(
    "search"
  );
  const [searchQuery, setSearchQuery] = useState("");
  const [isSearching, setIsSearching] = useState(false);
  ...
  const searchAssets = async (query: string) => {
    ...
    try {
      const res = await fetch(
        `/api/asset-search?q=${encodeURIComponent(query)}`
      );
  ...
  return (
    ...
    >
      ...
      <DialogContent className="bg-ink-raised sm:max-w-xl">
        <DialogHeader>
          <DialogTitle className="text-primary">Add Asset</DialogTitle>
        </DialogHeader>
        ...
        <Tabs
          ...
          className="w-full"
        >
          <TabsList className="w-full bg-transparent border-b border-ink-edge p-0 h-auto rounded-none gap-0">
            <TabsTrigger
              value="search"
              className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-accent-core data-[state=active]:text-primary text-muted py-3 bg-transparent data-[state=active]:bg-transparent hover:text-body transition-colors"
            >
          ...
          <TabsContent value="search" className="mt-4">
            {!selectedAsset ? (
              <div className="space-y-4">
                ...
                {hasResults && (
                  <div className="border border-ink-edge rounded-xl overflow-hidden">
                    {stockResults.length > 0 && (
                      <>
                        <div className="px-3 py-2 bg-ink-raised text-xs text-secondary font-medium flex items-center gap-2 border-b border-ink-edge">
                          <Building2 className="w-3 h-3" /> Stocks
                        ...
                        {stockResults.slice(0, 5).map((result) => (
                          <button
                            key={result.symbol}
                            onClick={() =>
                              handleSelectAsset({ ...result, type: "stock" })
                            }
                            className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-accent/40 text-left border-b border-ink-edge last:border-b-0 transition-colors"
                          >
                            <div className="w-9 h-9 rounded-xl bg-accent-core/10 flex items-center justify-center">
                              <Building2 className="w-4 h-4 text-accent-bright" />
                            </div>
          ...
          <TabsContent value="cash" className="mt-4">
            <div className="space-y-4">
              <div>
                <Label className="text-secondary text-sm">Type</Label>
                <Select
                  value={assetType}
                  onValueChange={(v) => {
                    setAssetType(v);
          ...
          <TabsContent value="retirement" className="mt-4">
            <div className="space-y-4">
              ...
              <div>
                <Label className="text-secondary text-sm">Account Name</Label>
                <Input
                  value={accountName}
                  onChange={(e) => {
              ...
              <div className="p-3 bg-ink-surface rounded-xl">
                <Label className="text-body text-sm font-medium flex items-center gap-2 mb-3">
                  <TrendingUp className="w-4 h-4 text-gain" />
              ...
              <Button
                onClick={() => {
                  if (!quantity || !accountName) {
                    toast({
                      title: "Enter balance and account name",
                      variant: "destructive",
                    });
                    return;
                  }
          ...
          <TabsContent value="manual" className="mt-4">
            <div className="space-y-4">
              <div>
                <Label className="text-secondary text-sm">Asset Type</Label>
                <Select value={assetType} onValueChange={setAssetType}>
                  <SelectTrigger className="h-11 mt-1.5 focus-visible:ring-2 focus-visible:ring-ring">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-secondary text-sm">Symbol</Label>
                  <Input
                    value={symbol}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label className="text-secondary text-sm">Avg Cost ($)</Label>
                  <Input
                    type="number"
                    value={avgCost}
          ...
          {selectedAsset && mode === "search" && (
            <div className="mt-4 space-y-4">
              <div className="flex items-center gap-3 p-3 bg-ink-surface rounded-xl">
                {selectedAsset.thumb ? (
                  <img
                  ...
                  >
                    {selectedAsset.type === "crypto" ? (
                      ...
                      <Building2 className="w-5 h-5 text-accent-bright" />
                    )}
                  </div>
                )}
                <div className="flex-1">
...
const AssetRow = memo(function AssetRow({
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-between p-3 bg-ink-surface rounded-xl border hover:border-ink-edge hover:bg-accent/40 transition-colors cursor-pointer group relative",
        isRecentlyUpdated ? "border-gain/40" : "border-ink-edge/30"
      )}
    >
      {isRecentlyUpdated && (
        <motion.div
          ...
          className="absolute -right-1 -top-1 w-3 h-3 rounded-full bg-gain"
        />
      )}
      <div className="flex items-center gap-3">
        <div
          className={cn("p-2 rounded-xl bg-ink-raised relative", colorGradient)}
        >
          <Icon className="w-4 h-4 text-primary" />
          {isRecentlyUpdated && (
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] sm:text-[10px] text-muted uppercase">
              {asset.assetType}
            </span>
          </div>
          <p className="text-xs text-muted truncate max-w-[120px]">
        ...
        <div className="flex items-center justify-end gap-2">
          {!isCashLike && (
            <div
              className={cn(
                ...
                is24hPositive ? "text-gain" : "text-loss"
              )}
            >
              {is24hPositive ? (
                <ArrowUpRight className="w-2.5 h-2.5" />
          ...
          {!isCashLike &&
            ...
            asset.unrealizedPnlPercent !== null && (
              ...
              >
                PnL {isPnlPositive ? "+" : ""}
                {(asset.unrealizedPnlPercent || 0).toFixed(0)}%
              </span>
            )}
          {isCashLike && <span className="text-[10px] text-muted">Cash</span>}
          {isRetirement && asset.unrealizedPnl !== null && (
            <span
              className={cn(
                "text-[10px] px-1 py-0.5 rounded",
                isPnlPositive ? "bg-gain/20 text-gain" : "bg-loss/20 text-loss"
              )}
            >
              {isPnlPositive ? "+" : ""}$
              {Math.abs(asset.unrealizedPnl || 0).toLocaleString(undefined, {
                maximumFractionDigits: 0,
              })}
            </span>
          )}
...
function EditAssetDialog({
  ...
  const [avgCost, setAvgCost] = useState(
    asset.averageCostBasis?.toString() || ""
  );
  ...
  const recalculateMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(
        `/api/portfolios/${portfolioId}/assets/${asset.id}/recalculate`,
        {
          method: "POST",
          body: JSON.stringify({}),
        }
    ...
    onSuccess: (data: any) => {
      toast({
        ...
        description: data.message || "Asset price and chart updated",
      });
      queryClient.invalidateQueries({
        queryKey: ["/api/portfolios", portfolioId],
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-1/2 -translate-y-1/2 h-7 w-7 p-0 bg-ink-raised hover:bg-accent text-secondary hover:text-foreground"
          onClick={(e) => {
            e.stopPropagation();
            setOpen(true);
          }}
      ...
      <DialogContent className="bg-ink-raised sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between p-3 bg-ink-surface rounded-xl">
            <div>
              <p className="font-medium text-primary">{asset.symbol}</p>
              <p className="text-sm text-secondary">{asset.name}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-secondary">Current Price</p>
              <p className="font-medium text-primary">
                $
          ...
          <div>
            <Label className="text-secondary text-sm">Account (optional)</Label>
            <Input
              value={accountName}
              onChange={(e) => setAccountName(e.target.value)}
              placeholder="Coinbase, Fidelity..."
          ...
          <div className="p-3 bg-ink-surface rounded-xl">
            <div className="flex justify-between text-sm">
              <span className="text-secondary">Est. Value</span>
              <span className="text-primary font-medium">
                $
                {(
                  parseFloat(quantity || "0") * (asset.currentPrice || 0)
                ).toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </span>
            </div>
            {!isCashLike && avgCost && (
              <div className="flex justify-between text-sm mt-1">
                <span className="text-secondary">Est. PnL</span>
                <span
                  className={cn(
                    "font-medium",
                    parseFloat(quantity || "0") * (asset.currentPrice || 0) -
                ...
                >
                  ...
                  {(
                    parseFloat(quantity || "0") * (asset.currentPrice || 0) -
                    parseFloat(quantity || "0") * parseFloat(avgCost || "0")
                  ).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                </span>
              </div>
            )}
          ...
          <div className="flex flex-col gap-2 pt-2">
            ...
            {!isCashLike && (
              ...
              >
                {recalculateMutation.isPending ? (
                  ...
                  <>
                    <RefreshCw className="w-4 h-4 mr-2" /> Fix Price & Chart
                  </>
                )}
              </Button>
            )}
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1 border-loss/30 text-loss hover:bg-loss/10 hover:text-loss"
...
export default function PortfolioDashboard() {
  ...
  const { data: tradeSignalsData } = useQuery<{
    signals: Array<{
      type: string;
      symbol: string;
      action: string;
  ...
  const syncMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/portfolios/${activePortfolioId}/sync`, {
        method: "POST",
      });
    },
    onSuccess: () => {
  ...
  const createPortfolioMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Start adding your assets now.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const {
    ...
    prices: livePrices,
    recentUpdates,
  } = useWebSocketPrices(assetSymbols);
      ...
      if (livePrice && livePrice.price > 0) {
        ...
        const newUnrealizedPnlPercent =
          asset.totalCostBasis > 0
            ? (newUnrealizedPnl / asset.totalCostBasis) * 100
            : 0;
        return {
          ...asset,
  ...
  useEffect(() => {
    if (activePortfolioId && rawAssets.length > 0 && !syncMutation.isPending) {
      ...
      const needsSync =
        !lastUpdated || Date.now() - new Date(lastUpdated).getTime() > 300000;
      if (needsSync) {
        syncMutation.mutate();
      }
    }
  ...
  if (portfoliosLoading) {
    return (
      <div className="min-h-screen bg-ink-page    pt-20 px-4 tnums-scope">
        <div className="max-w-7xl mx-auto">
          <div className="animate-pulse space-y-6">
            <div className="h-12 bg-ink-raised rounded-xl w-1/3" />
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-32 bg-ink-raised rounded-xl" />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-ink-page    pt-20 pb-8 px-3 sm:px-4 overflow-x-hidden tnums-scope">
      <div className="max-w-7xl mx-auto">
        <motion.div
        ...
        >
            ...
            actions={
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-end">
                ...
                {assets.length > 0 && showValues && (
                  <div
                    className={cn(
                      "px-2 sm:px-3 py-1.5 rounded-xl text-sm font-medium flex items-center gap-1.5",
                      isPnlPositive
                        ? "bg-gain/10 text-gain border border-gain/20"
                        : "bg-loss/10 text-loss border border-loss/20"
                    )}
                ...
                <Button
                  ...
                  className="text-secondary hover:text-foreground hover:bg-accent/40 min-w-[44px] min-h-[44px]"
                >
                  {showValues ? (
                ...
                <div className="flex flex-col items-end">
                  <Button
                    ...
                    className="border-ink-edge text-foreground hover:bg-accent/40 min-h-[44px]"
                  ...
                  {assets[0]?.priceLastUpdated && (
                    <span className="text-[11px] sm:text-[10px] text-muted mt-0.5 hidden sm:block">
                      Updated{" "}
                      {new Date(assets[0].priceLastUpdated).toLocaleTimeString(
                        [],
          ...
          />
          <div className="h-px bg-ink-raised    mt-4" />
          ...
          {portfolios.length === 0 ? (
            ...
            >
              <Surface className="bg-ink-raised p-8">
                <div className="text-center mb-8">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-ink-raised   flex items-center justify-center">
                  ...
                  <p className="text-secondary">
                    Track all your assets in one place with AI-powered insights.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-ink-edge/60">
                  {[
                    {
                      icon: PieChart,
                      label: "Track All Assets",
                      desc: "Crypto, stocks, cash",
                    },
                    {
                      icon: Brain,
                    ...
                    <div key={feature.label} className="text-center">
                      <feature.icon className="w-6 h-6 mx-auto mb-2 text-accent-bright" />
                      <p className="text-sm font-medium text-primary">
                        {feature.label}
                      </p>
            ...
            <>
                ...
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                  {portfolios.map((p) => (
                    <Button
                      key={p.id}
                      variant={
                        p.id === activePortfolioId ? "default" : "outline"
                      }
                      size="sm"
              ...
              <StatGrid className="mb-6">
                ...
                <Surface className="relative overflow-hidden bg-ink-raised    border-accent-core/20 p-5 hover:border-accent-core/40 transition-all hover:shadow-lg hover:shadow-purple-500/10">
                  ...
                  <div className="relative">
                    ...
                    <div
                      className={cn(
                        "flex items-center gap-1.5 text-sm mt-2 font-medium",
                ...
                {/* Cost Basis Card */}
                <Surface className="bg-ink-raised surface-interactive p-5 hover:border-neon-cyan/50">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="p-2 rounded-xl bg-accent-core/10">
                      <Target className="w-4 h-4 text-accent-bright" />
                    </div>
                    <span className="text-secondary text-sm font-medium">
                      Invested
                    </span>
                  ...
                  <p className="text-2xl font-bold text-primary">
                    {showValues
                      ? `$${(portfolio?.totalCostBasis || 0).toLocaleString(
                          undefined,
                          { maximumFractionDigits: 0 }
                        )}`
                      : "••••••"}
                  </p>
                  <p className="text-xs text-muted mt-2">Total cost basis</p>
                </Surface>
                ...
                <Surface className="bg-ink-raised surface-interactive p-5 hover:border-emerald-500/50">
                  ...
                  <p className="text-xs text-muted mt-2">Positions tracked</p>
                </Surface>
                  ...
                  <div className="relative">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="p-2 rounded-xl bg-warn/20">
                        <Brain className="w-4 h-4 text-warn" />
                      </div>
                      <span className="text-secondary text-sm font-medium">
                        AI Score
                      </span>
                    ...
                    <Badge
                      className={cn(
                        "mt-2 text-xs",
                        riskLevelColors[
                          analysis?.riskLevel || portfolio?.riskLevel || ""
                        ] || "text-secondary bg-ink-raised/20"
                      )}
                    >
                      {(
                        analysis?.riskLevel ||
              ...
              {/* Quick Actions Bar */}
              <Surface className="bg-ink-surface p-3 mb-6">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Button
                      ...
                      className="text-accent-bright border-accent-core/30 hover:bg-accent-core/10"
                    ...
                    <Button
                      ...
                      className="text-secondary hover:text-foreground hover:bg-accent/40 hidden sm:flex"
                    ...
                    <Button
                      ...
                      className="text-secondary hover:text-foreground hover:bg-accent/40 hidden md:flex"
                    ...
                    <Button
                      ...
                      className="text-secondary hover:text-foreground hover:bg-accent/40 hidden lg:flex"
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="outline"
                      className="text-secondary border-ink-edge text-xs"
                    >
                      <Radio className="w-3 h-3 mr-1 text-gain" />
                      Live
                    </Badge>
                    <span className="text-xs text-muted">
                      {assets[0]?.priceLastUpdated
              ...
              {activePortfolioId && assets.length > 0 && (
                <Surface className="bg-ink-raised p-6 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-primary flex items-center gap-2">
                      <LineChart className="w-5 h-5 text-accent-bright" />
                      Net Worth History
                    </h2>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                  <Surface className="bg-ink-raised p-6">
                    <Tabs defaultValue="holdings" className="w-full">
                      <div className="flex items-center justify-between mb-4">
                        <TabsList className="bg-transparent border-none p-0 h-auto">
                          <TabsTrigger
                            value="holdings"
                            className="data-[state=active]:bg-transparent data-[state=active]:text-primary text-muted px-0 mr-4"
                          ...
                          >
                            ...
                            <Badge
                              variant="outline"
                              className="ml-2 text-secondary border-ink-edge text-xs"
                            >
                              {assets.length}
                            </Badge>
                          </TabsTrigger>
                          <TabsTrigger
                            value="watchlist"
                            className="data-[state=active]:bg-transparent data-[state=active]:text-primary text-muted px-0"
                          ...
                          <TabsTrigger
                            value="bot-trading"
                            className="data-[state=active]:bg-transparent data-[state=active]:text-primary text-muted px-0 ml-4"
                      ...
                      <TabsContent value="holdings" className="mt-0">
                        {assets.length === 0 ? (
                          <div className="py-8">
                            <div className="text-center max-w-sm mx-auto">
                              <motion.div
                                className="w-20 h-20 mx-auto mb-5 rounded-2xl bg-ink-raised    flex items-center justify-center relative"
                                animate={{
                                  boxShadow: [
                              ...
                              >
                                <Layers className="w-10 h-10 text-accent-bright" />
                                <motion.div
                                  className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-accent-core"
                              ...
                              <p className="text-secondary text-sm mb-6">
                                Track crypto, stocks, ETFs, retirement accounts,
                                and cash in one unified dashboard with
                                AI-powered insights.
                              </p>
                              <div className="grid grid-cols-3 gap-3 mb-6">
                                {[
                                  ...
                                  <div
                                    key={item.label}
                                    className="p-3 bg-ink-surface rounded-xl"
                                  >
                                    <item.icon
                                      className={cn(
                          ...
                          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                            {assets.map((asset) => (
                              <AssetRow
                                ...
                                isRecentlyUpdated={recentUpdates.has(
                  ...
                  <Surface className="bg-ink-raised p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-lg font-semibold text-primary flex items-center gap-2">
                        <LineChart className="w-5 h-5 text-gain" />
                        Performance
                      </h2>
                    </div>
                    <PerformanceSummary
                  ...
                  <Surface className="bg-ink-raised p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-lg font-semibold text-primary flex items-center gap-2">
                        <Gauge className="w-5 h-5 text-accent-bright" />
                        Risk Metrics
                      </h2>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div className="p-3 bg-ink-surface rounded-xl">
                        <div className="flex items-center gap-2 mb-2">
                          <Crosshair className="w-4 h-4 text-warn" />
                          <span className="text-xs text-secondary">
                            Concentration
                          </span>
                        </div>
                  ...
                  <Surface className="bg-ink-raised p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] text-accent-bright border-accent-core/30"
                      >
                        Alpha
                      </Badge>
                    </div>
                    {tradeSignals.length > 0 ? (
                      ...
                      <div className="text-center py-6">
                        <Zap className="w-8 h-8 mx-auto text-muted mb-2" />
                        <p className="text-sm text-secondary">
                          Add assets to receive personalized trade signals
                        </p>
                      </div>
                    )}
                  </Surface>
                  ...
                  <Surface className="bg-ink-raised p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] text-secondary border-ink-edge"
                      >
                        30D
                      </Badge>
                    </div>
                  ...
                  <Surface className="bg-ink-raised p-6">
                    ...
                    analysis.recommendations.length > 0 ? (
                        ...
                        {analysis.recommendations
                          ...
                          .map((rec, index) => (
                            <div
                              key={index}
                              className={cn(
                                "p-3 rounded-xl border transition-colors cursor-pointer hover:border-accent-core/30",
                                rec.priority === "high"
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                >
                                  {rec.priority === "high" ? (
                                    <Flame className="w-3.5 h-3.5 text-loss" />
                                  ) : rec.priority === "medium" ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-2">
                                    <Badge
                                      variant="outline"
                                      className="text-[11px] sm:text-[10px] text-secondary border-ink-edge px-1.5 py-0"
                                    >
                                      {rec.type}
                                    </Badge>
                                    {rec.action && (
                                      <span className="text-xs text-accent-bright hover:text-accent-bright">
                                        {rec.action} →
                                      </span>
                                    )}
                                  </div>
                      ...
                      <div className="text-center py-6">
                        <div className="w-10 h-10 mx-auto mb-3 rounded-xl bg-warn/10 flex items-center justify-center">
                          <Sparkles className="w-5 h-5 text-warn" />
                        </div>
                        <p className="text-sm text-secondary">
                          Add assets to receive AI-powered insights
                        </p>
                      </div>
                    )}
                  </Surface>
                ...
                <div className="space-y-6">
                  ...
                  <Surface className="bg-ink-raised p-6">
                    ...
                    <div className="flex justify-center mb-5">
                      <HealthScoreRing
                        score={
                          analysis?.healthScore || portfolio?.healthScore || 0
                        }
                      />
                    </div>
                    <div className="space-y-3">
                  ...
                  {/* Connected Accounts */}
                  <Surface className="bg-ink-raised p-5">
                  ...
                  <Surface className="bg-ink-raised p-6">
                    ...
                    {analysis?.allocation &&
                    Object.keys(analysis.allocation).length > 0 ? (
                      <>
                        <div className="flex justify-center mb-4">
                          <AllocationChart allocation={analysis.allocation} />
                        </div>
                        <div className="space-y-2">
                          {Object.entries(analysis.allocation).map(
                            ([type, percent]) => {
                              // Use same colors as the chart for consistency
                              const chartColor =
                                assetTypeChartColors[type] ||
                                assetTypeChartColors.other;
                              return (
                                <div
                                  key={type}
                      ...
                      <div className="text-center py-6">
                        <div className="w-10 h-10 mx-auto mb-3 rounded-xl bg-accent-core/10 flex items-center justify-center">
                          <PieChart className="w-5 h-5 text-accent-bright" />
                        </div>
                        <p className="text-sm text-secondary">
                  ...
                  <Surface className="bg-ink-raised    border-loss/20 p-5">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="p-2 rounded-xl bg-loss/20">
                    ...
                    <div className="space-y-2 mb-3">
                      {(() => {
                        const assetTypes = Array.from(
                          new Set(assets.map((a) => a.assetType))
                        );
                        const impactByType = assetTypes
                          .map((type) => {
                        ...
                        const totalImpact = impactByType.reduce(
                          (sum, t) => sum + t.impact,
                          0
                        );
                        ...
                        return (
                          <>
                            {impactByType.slice(0, 3).map((t) => (
                              <div
                                key={t.type}
                                className="flex justify-between items-center p-2 bg-ink-surface rounded-xl"
                              >
                                <span className="text-xs text-secondary capitalize">
                                  If {t.type} drops {t.dropPercent}%
                                </span>
                            ...
                            <div className="flex justify-between items-center p-2 bg-loss/10 rounded-xl border border-loss/20">
                              <span className="text-xs text-body font-medium">
                                Total Custom Scenario Impact
                              </span>
                    ...
                    <Button
                      variant="outline"
                      className="w-full border-loss/30 text-loss hover:bg-loss/10 text-xs h-8"
                    ...
                    <div className="space-y-2 max-h-[200px] overflow-y-auto">
                      {portfolioEvents.length > 0 ? (
                        portfolioEvents.slice(0, 5).map((event) => {
                          ...
                          const colors =
                            eventColors[event.type] || eventColors.fed;
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-xs text-body">{event.title}</p>
                              {event.description && (
                        ...
                        <p className="text-xs text-muted text-center py-4">
                          Add assets to see relevant events
                        </p>
                      ) : (
                        <p className="text-xs text-muted text-center py-4">
                  ...
                  {/* Goal Tracker */}
                  <Surface className="bg-ink-raised    border-accent-core/20 p-5">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-semibold text-primary text-sm flex items-center gap-2">
                        <Target className="w-4 h-4 text-accent-bright" />
                        Goals
                  ...
                  <Surface className="bg-ink-raised p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[11px] sm:text-[9px] text-secondary border-ink-edge"
                      >
                        Live
                      </Badge>
                    </div>
              ...
              <div className="relative py-8">
                <div className="absolute inset-0 flex items-center">
                  <div className="w-full border-t border-ink-edge/60" />
                </div>
              ...
              <div className="mt-6 space-y-6">
                ...
                <Surface className="bg-ink-raised    border-accent-core/30 p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <Badge className="bg-ink-raised   border-accent-core/30 text-accent-bright">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-2 p-4 bg-ink-surface rounded-xl">
                      <div className="flex items-start gap-3">
                        {(() => {
                          const sortedByAlloc = [...assets].sort(
                            (a, b) =>
                              (b.allocationPercent || 0) -
                              (a.allocationPercent || 0)
                          );
                          const topAsset = sortedByAlloc[0];
                          const losers = assets.filter(
                            (a) => (a.unrealizedPnlPercent || 0) < -10
                          );
                          const bigWinners = assets.filter(
                            (a) => (a.unrealizedPnlPercent || 0) > 30
                          );
                          const cryptoAlloc = assets
                            .filter((a) => a.assetType === "crypto")
                            .reduce(
                              (s, a) => s + (a.allocationPercent || 0),
                              0
                            );
                          const cashAlloc = assets
                          ...
                          if (assets.length === 0) {
                            return (
                              <>
                                <div className="p-2 rounded-xl bg-accent-core/20 mt-0.5">
                                  <Target className="w-4 h-4 text-accent-bright" />
                                </div>
                                <div className="flex-1">
                                  ...
                                  <p className="text-sm text-secondary mb-3">
                                    Add your first assets to receive
                          ...
                          if (
                            taxAnalytics?.taxLossHarvestingOpportunities &&
                            taxAnalytics.taxLossHarvestingOpportunities
                              .length >= 2
                          ) {
                            const topLoss =
                              taxAnalytics.taxLossHarvestingOpportunities[0];
                            return (
                              <>
                                <div className="p-2 rounded-xl bg-warn/20 mt-0.5">
                                  <Receipt className="w-4 h-4 text-warn" />
                                </div>
                                <div className="flex-1">
                                  <h4 className="font-medium text-primary mb-1">
                                    ...
                                    {showValues
                                      ? `You have ${
                                          taxAnalytics
                                            ...
                                            .length
                                        } positions with unrealized losses. Selling ${
                                          topLoss.symbol
                                        } could save ~$${topLoss.potentialTaxSavings.toLocaleString(
                                          undefined,
                                          { maximumFractionDigits: 0 }
                                        )} in taxes this year.`
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      size="sm"
                                      onClick={() => setShowTaxDialog(true)}
                                      className="bg-warn/20 text-warn hover:bg-warn/30 h-7 text-xs"
                                    >
                          ...
                          if (
                            topAsset &&
                            (topAsset.allocationPercent || 0) > 35
                          ) {
                            return (
                              <>
                                <div className="p-2 rounded-xl bg-loss/20 mt-0.5">
                                  <AlertTriangle className="w-4 h-4 text-loss" />
                                </div>
                                <div className="flex-1">
                                  <h4 className="font-medium text-primary mb-1">
                                  ...
                                  <p className="text-sm text-secondary mb-3">
                                    {showValues
                                      ? `Your ${
                                          ...
                                          topAsset.allocationPercent || 0
                                        ).toFixed(
                                          0
                                        )}% of your portfolio. Consider rebalancing to reduce single-asset risk.`
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      className="bg-accent-core/20 text-accent-bright hover:bg-accent-core/30 h-7 text-xs"
                                    >
                          ...
                          if (bigWinners.length > 0) {
                            const topWinner = bigWinners.sort(
                              (a, b) =>
                                (b.unrealizedPnlPercent || 0) -
                                (a.unrealizedPnlPercent || 0)
                            )[0];
                            return (
                              <>
                                <div className="p-2 rounded-xl bg-gain/20 mt-0.5">
                                  <TrendingUp className="w-4 h-4 text-gain" />
                                </div>
                                <div className="flex-1">
                                  <h4 className="font-medium text-primary mb-1">
                                  ...
                                  <p className="text-sm text-secondary mb-3">
                                    {showValues
                                      ? `${topWinner.symbol} is up ${(
                                          topWinner.unrealizedPnlPercent || 0
                                        ).toFixed(0)}% (+$${(
                                          topWinner.unrealizedPnl || 0
                                        ).toLocaleString(undefined, {
                                          maximumFractionDigits: 0,
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      className="bg-gain/20 text-gain hover:bg-gain/30 h-7 text-xs"
                                    >
                          ...
                          if (cryptoAlloc > 50) {
                            return (
                              <>
                                <div className="p-2 rounded-xl bg-accent-core/20 mt-0.5">
                                  <Gauge className="w-4 h-4 text-accent-bright" />
                                </div>
                                <div className="flex-1">
                                  <h4 className="font-medium text-primary mb-1">
                                  ...
                                  <p className="text-sm text-secondary mb-3">
                                    {cryptoAlloc.toFixed(0)}% of your portfolio
                                    is in crypto. Consider diversifying with
                                    stocks or ETFs for a more balanced risk
                                    profile.
                                  </p>
                                  <div className="flex items-center gap-2">
                                    <Button
                          ...
                          return (
                            <>
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-secondary mb-3">
                                  Your portfolio is well-diversified with
                                  balanced allocations. Keep monitoring for
                    ...
                    <div className="space-y-3">
                      <button
                        onClick={() => setShowTaxDialog(true)}
                        className="w-full p-3 bg-ink-surface surface-interactive rounded-xl hover:border-neon-amber/50 text-left group"
                        data-testid="button-tax-harvest"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <Percent className="w-3.5 h-3.5 text-warn" />
                          <span className="text-xs font-medium text-primary">
                            Tax-Loss Harvest
                          </span>
                        </div>
                        <p className="text-[11px] sm:text-[10px] text-muted group-hover:text-secondary">
                          {
                            assets.filter((a) => (a.unrealizedPnl || 0) < 0)
                              .length
                          }{" "}
                          assets with losses
                        </p>
                      ...
                      <button
                        onClick={() => setShowRebalanceDialog(true)}
                        className="w-full p-3 bg-ink-surface surface-interactive rounded-xl hover:border-neon-cyan/50 text-left group"
                        data-testid="button-auto-rebalance"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <Scale className="w-3.5 h-3.5 text-accent-bright" />
                          <span className="text-xs font-medium text-primary">
                            Auto-Rebalance
                          </span>
                        </div>
                        <p className="text-[11px] sm:text-[10px] text-muted group-hover:text-secondary">
                          Optimize allocation targets
                        </p>
                      </button>
                      <button
                        onClick={() => setShowGoalsDialog(true)}
                        className="w-full p-3 bg-ink-surface surface-interactive rounded-xl hover:border-neon-purple/50 text-left group"
                      ...
                      >
                        ...
                        <p className="text-[10px] text-muted group-hover:text-secondary">
                          Track your targets
                        </p>
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-3 bg-ink-surface rounded-xl">
                        ...
                        <p
                          className={cn(
                            "text-[10px]",
                            Math.abs(analytics?.maxDrawdown || 0) > 25
                              ? "text-loss"
                              : Math.abs(analytics?.maxDrawdown || 0) > 15
                              ? "text-warn"
                              : "text-gain"
                          )}
                        >
                      ...
                      <div className="p-3 bg-ink-surface rounded-xl">
                        <div className="flex items-center gap-2 mb-2">
                          <Activity className="w-4 h-4 text-accent-bright" />
                          <span className="text-xs text-secondary">Beta</span>
                        </div>
                        <p className="text-xl font-bold text-primary">
                          {assets.length > 0
                        ...
                        <p
                          className={cn(
                            "text-[10px]",
                            (analytics?.beta || 1) > 1.2
                              ? "text-loss"
                              : (analytics?.beta || 1) > 0.8
                              ? "text-muted"
                              : "text-gain"
                          )}
                        >
                      ...
                      <div className="p-3 bg-ink-surface rounded-xl">
                        <div className="flex items-center gap-2 mb-2">
                          <Crosshair className="w-4 h-4 text-accent-bright" />
                          <span className="text-xs text-secondary">Alpha</span>
                        </div>
                        <p
                          className={cn(
                            "text-xl font-bold",
                    ...
                    <div
                      className={cn(
                        ...
                        (analytics?.outperformance || 0) >= 0
                          ? "bg-ink-raised   border-gain/20"
                          : "bg-ink-raised   border-loss/20"
                      )}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          >
                            {showValues && assets.length > 0
                              ? `${
                                  ...
                                  analytics?.outperformance?.toFixed(1) || "0"
                                }% ${
                                  (analytics?.outperformance || 0) >= 0
                                    ? "outperformance"
                                    : "underperformance"
                        ...
                        <div className="text-right">
                          <p className="text-xs text-muted">
                            ...
                            {showValues && assets.length > 0
                              ? `${
                                  (analytics?.ytdReturn || 0) >= 0 ? "+" : ""
                                }${analytics?.ytdReturn?.toFixed(1) || "0"}%`
                              : assets.length > 0
                              ? "••••"
                              : "--"}
                          </p>
                          <p className="text-xs text-muted">
                            S&P 500: +{analytics?.spReturn || 19.7}%
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <Surface className="bg-ink-raised p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="p-3 bg-ink-surface rounded-xl">
                        ...
                        <p className="text-xl font-bold text-warn">
                          {showValues
                            ? `$${Math.round(
                                taxAnalytics?.totalEstTax ||
                                  (portfolio?.totalPnl || 0) * 0.2
                              ).toLocaleString()}`
                            : "••••"}
                        </p>
                        <p className="text-[9px] text-muted mt-0.5">
                          {taxAnalytics
                            ? `15% long-term • 32% short-term`
                            : "~20% blended rate"}
                        </p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between p-2 bg-ink-surface rounded-xl">
                        ...
                        <span className="text-xs font-medium text-primary">
                          {taxAnalytics
                            ? `${taxAnalytics.longTermAssetCount} assets`
                            : assets.length > 0
                            ? "--"
                            : "--"}
                        </span>
                      ...
                      <div className="flex items-center justify-between p-2 bg-ink-surface rounded-xl">
                        ...
                        <span className="text-xs font-medium text-primary">
                          {taxAnalytics
                            ? `${taxAnalytics.shortTermAssetCount} assets`
                            : assets.length > 0
                            ? "--"
                            : "--"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between p-2 bg-loss/5 border border-loss/20 rounded-xl">
                        <div className="flex items-center gap-2">
                      ...
                      {taxAnalytics?.taxLossHarvestingOpportunities &&
                        taxAnalytics.taxLossHarvestingOpportunities.length >
                          0 && (
                          <div className="mt-2 p-2 bg-loss/5 rounded-xl border border-loss/10">
                            <p className="text-[10px] text-secondary mb-1.5">
                              Top loss harvest candidates:
                            </p>
                                ...
                                .map((op, i) => (
                                  ...
                                  >
                                    ...
                                    <span className="text-[10px] text-loss">
                                      ${Math.abs(op.loss).toLocaleString()} loss
                                      → ~$
                                      {op.potentialTaxSavings.toLocaleString(
                                        undefined,
                                        { maximumFractionDigits: 0 }
                                      )}{" "}
                                      savings
                ...
                <Surface className="bg-ink-raised p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-primary flex items-center gap-2">
                      <Calendar className="w-5 h-5 text-gain" />
                      Dividend Calendar
                    </h2>
                    <Badge
                      variant="outline"
      ...
      <Dialog open={showAlertDialog} onOpenChange={setShowAlertDialog}>
        <DialogContent className="bg-ink-raised max-w-md max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="space-y-4 pt-4">
            <div className="p-4 bg-ink-surface rounded-xl">
              <h4 className="text-sm font-medium text-primary mb-3">
                Create New Alert
              </h4>
              <div className="space-y-3">
                <div>
                  <Label className="text-secondary text-xs">Select Asset</Label>
                  <Select value={alertSymbol} onValueChange={setAlertSymbol}>
                    <SelectTrigger
                      className="mt-1"
                      data-testid="select-alert-asset"
                    >
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    >
                      ...
                      <SelectContent className="bg-ink-raised">
                        <SelectItem value="above" className="text-primary">
                          Goes above
                        </SelectItem>
                        <SelectItem value="below" className="text-primary">
                          Goes below
                        </SelectItem>
                      </SelectContent>
                    </Select>
                ...
                <Button
                  onClick={() => {
                    const selectedAsset = assets.find(
                      (a) => a.symbol === alertSymbol
                    );
                    if (!selectedAsset || !alertTargetPrice) {
                      toast({
                        title:
                          "Please select an asset and enter a target price",
                        variant: "destructive",
                      });
                      return;
                    }
                      ...
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {alert.isTriggered && (
                              <Badge className="text-[10px] bg-warn/20 text-warn">
                                Triggered
                              </Badge>
                            )}
                          </div>
                          <p className="text-xs text-secondary mt-0.5">
                            Target: ${alert.targetPrice?.toLocaleString()} (was
                            ${alert.currentPriceAtCreation?.toLocaleString()})
                          </p>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-7 w-7 p-0 text-secondary hover:text-loss"
        ...
        onOpenChange={(open) => {
          ...
          if (open && Object.keys(targetAllocations).length === 0) {
            ...
            assetTypes.forEach((type) => {
              initialTargets[type] = Math.round(equalAlloc);
            });
            setTargetAllocations(initialTargets);
          }
        }}
      >
        <DialogContent className="bg-ink-raised max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4 pt-4">
            <div className="p-4 bg-ink-surface rounded-xl">
              <h4 className="text-sm font-medium text-primary mb-3">
                Set Target Allocations by Asset Type
              </h4>
              <div className="space-y-2">
                {Array.from(new Set(assets.map((a) => a.assetType))).map(
                  (type) => {
                    const currentAlloc = assets
                      .filter((a) => a.assetType === type)
                      .reduce((sum, a) => sum + (a.allocationPercent || 0), 0);
                    return (
                      <div key={type} className="flex items-center gap-3">
                        <span className="text-sm text-body capitalize flex-1 min-w-[80px]">
                          {type}
                        </span>
                        <span className="text-xs text-muted w-16">
                          Now: {currentAlloc.toFixed(1)}%
                        </span>
                ...
                <div className="pt-2 border-t border-ink-edge/60 flex justify-between text-xs">
                  <span className="text-secondary">Total:</span>
                  <span
                    className={cn(
                      "font-medium",
                      Object.values(targetAllocations).reduce(
                        (a, b) => a + b,
                        0
                      ) === 100
                        ? "text-gain"
                ...
                {(() => {
                  ...
                  assets.forEach((asset) => {
                    ...
                    if (Math.abs(diff) > 1) {
                      ...
                      const shares = asset.currentPrice
                        ? dollarAmount / asset.currentPrice
                        : 0;
                      trades.push({
                        symbol: asset.symbol,
                  ...
                  if (trades.length === 0) {
                    return (
                      <div className="text-center py-4 text-secondary text-sm">
                        <CheckCircle className="w-6 h-6 mx-auto mb-2 text-gain" />
                  ...
                  return trades.map((trade, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        >
                          $
                          {trade.amountDollars.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}
                        </span>
                      </div>
                      <p className="text-xs text-secondary mt-1">
                        ...
                        {trade.amountPercent.toFixed(1)}% of portfolio)
                      </p>
                    </div>
                  ));
                })()}
            ...
            <Button
              onClick={() => {
                  ...
                  .map((asset) => {
                    ...
                    if (Math.abs(diff) > 1) {
                      return `${diff > 0 ? "BUY" : "SELL"} ${
                        ...
                        (Math.abs(diff) / 100) * (portfolio?.totalValue || 0)
                      ).toLocaleString()}`;
                    }
                    return null;
                  })
                ...
                toast({
                  ...
                  description:
                    trades.length > 0
                      ? trades.slice(0, 3).join(", ") +
                        (trades.length > 3
                          ? ` and ${trades.length - 3} more`
                          : "")
                      : "No trades needed",
                });
      ...
      <Dialog open={showTaxDialog} onOpenChange={setShowTaxDialog}>
        <DialogContent className="bg-ink-raised max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Receipt className="w-5 h-5 text-warn" />
              Tax Analytics
          ...
          <div className="space-y-5 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-ink-surface rounded-xl border border-gain/20">
                ...
                <p className="text-lg font-bold text-gain">
                  {taxAnalytics?.longTermAssetCount || 0} assets
                </p>
                <p className="text-xs text-muted">
                  {taxAnalytics?.longTermGains
                    ? `+$${taxAnalytics.longTermGains.toLocaleString(
                        undefined,
                        { maximumFractionDigits: 0 }
                      )} gains`
                    : "No gains"}
              ...
              <div className="p-3 bg-ink-surface rounded-xl border border-warn/20">
                ...
                <p className="text-lg font-bold text-warn">
                  {taxAnalytics?.shortTermAssetCount || 0} assets
                </p>
                <p className="text-xs text-muted">
                  {taxAnalytics?.shortTermGains
                    ? `+$${taxAnalytics.shortTermGains.toLocaleString(
                        undefined,
                        { maximumFractionDigits: 0 }
                      )} gains`
                    : "No gains"}
            ...
            {/* Estimated Tax */}
            <div className="p-4 bg-ink-raised   rounded-xl border border-warn/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-secondary">
            ...
            <div>
              ...
              {taxAnalytics?.taxLossHarvestingOpportunities &&
              taxAnalytics.taxLossHarvestingOpportunities.length > 0 ? (
                <div className="space-y-2">
                  {taxAnalytics.taxLossHarvestingOpportunities.map((op, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-3 bg-ink-surface rounded-xl border border-loss/20"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted">
                            {op.isLongTerm ? "Long-term" : "Short-term"} holding
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-loss">
                          -$
                          {Math.abs(op.loss).toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}
                        </p>
                        <p className="text-[10px] text-gain">
                          ~$
                          {op.potentialTaxSavings.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}{" "}
                          savings
                        </p>
                  ...
                  <p className="text-xs text-muted mt-2">
                    Selling these positions could offset capital gains and
                    reduce your tax bill.
                  </p>
                </div>
              ) : (
                <p className="text-center py-4 text-secondary bg-ink-surface rounded-xl">
      ...
      <Dialog open={showStrategyDialog} onOpenChange={setShowStrategyDialog}>
        <DialogContent className="bg-ink-raised max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Brain className="w-5 h-5 text-accent-bright" />
          ...
          <div className="space-y-4 pt-4">
            ...
            analysis.recommendations.length > 0 ? (
              analysis.recommendations.map((rec, i) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                      <p className="text-sm text-primary">{rec.message}</p>
                      {rec.action && (
                        <Button
                          size="sm"
                          variant="ghost"
                          className="mt-2 h-6 text-xs text-accent-bright"
                        >
                          {rec.action}
                        </Button>
                      )}
              ...
              <div className="text-center py-8">
                <Brain className="w-10 h-10 mx-auto text-muted mb-3" />
                <p className="text-secondary">
                  No recommendations yet. Add more assets to get AI insights.
                </p>
              </div>
            )}
          </div>
      ...
      <Dialog open={showGoalsDialog} onOpenChange={setShowGoalsDialog}>
        <DialogContent className="bg-ink-raised max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div>
              <Label className="text-secondary text-sm">Target Date</Label>
      ...
      <Dialog
        open={showStressTestDialog}
        onOpenChange={setShowStressTestDialog}
      >
        <DialogContent className="bg-ink-raised max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4 pt-4">
            <p className="text-sm text-secondary">
              Set a hypothetical market crash percentage for each asset type to
              see the potential impact on your portfolio.
            </p>
            ...
            <div className="space-y-3">
              {Object.entries(customScenario).map(([type, dropPercent]) => {
                ...
                if (
                  typeValue === 0 &&
                  !["crypto", "stock", "etf", "bond"].includes(type)
                )
                  return null;

                return (
                  <div key={type} className="p-3 bg-ink-surface rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {typeValue > 0 && (
                        <span className="text-xs text-muted">
                          Current: $
                          {typeValue.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}
                        </span>
                      )}
                    ...
                    {typeValue > 0 && impact > 0 && (
                      <p className="text-xs text-loss mt-2">
                        Impact: -$
                        {impact.toLocaleString(undefined, {
                          maximumFractionDigits: 0,
                        })}
                      </p>
                    )}
            ...
            <div className="p-4 bg-loss/10 rounded-xl border border-loss/30">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-primary">
              ...
              {(() => {
                const totalImpact = Object.entries(customScenario).reduce(
                  (total, [type, dropPercent]) => {
                ...
                const impactPercent =
                  portfolioValue > 0 ? (totalImpact / portfolioValue) * 100 : 0;
                const remainingValue = portfolioValue - totalImpact;

                return (
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <p className="text-xs text-secondary">Loss Amount</p>
                    ...
                    <div>
                      <p className="text-xs text-secondary">Loss %</p>
                      <p className="text-lg font-bold text-loss">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1 border-ink-edge text-secondary hover:bg-accent/40"
              ...
              <Button
                onClick={() => {
                  toast({
                    ...
                    description:
                      "Your custom stress test scenario has been applied",
                  });
