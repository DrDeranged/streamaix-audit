...
async function seedPredictionMarkets() {
  console.log('🌱 Starting prediction market seeding...\n');

  const privateKey = process.env.PRIVATE_KEY;
  if (!privateKey) {
    console.error('❌ PRIVATE_KEY environment variable not set!');
    console.log('📝 Please set your PRIVATE_KEY for Base network transactions');
    process.exit(1);
  }

  // Use a default creator wallet address (can be from PRIVATE_KEY or a test address)
  const creatorWallet = '0x1234567890123456789012345678901234567890'; // This will be derived from PRIVATE_KEY in production
  ...
  for (const seed of marketSeeds) {
    try {
      console.log(`📊 Creating market: "${seed.question.substring(0, 60)}..."`);
      
      const market = await predictionMarketService.createMarket({
        question: seed.question,
        description: seed.description,
        category: seed.category,
        creatorId: undefined as any, // System-created markets
        creatorWallet,
      ...
      console.log(`  ✅ Created market #${market.contractMarketId}: ${market.id}`);
