...
async function main() {
  ...
  if (balance === 0n) {
    ...
    console.error("📍 Please fund this address:", deployer.address);
    if (network.chainId === 84532n) {
      console.error("🚰 Get testnet ETH from: https://www.coinbase.com/faucets/base-ethereum-goerli-faucet");
    } else {
      console.error("💵 Send ETH to this address on Base mainnet");
    }
    process.exit(1);
  }
  ...
  // Set factory in ConditionalTokens
  console.log("\n3️⃣ Configuring ConditionalTokens...");
  const tx = await conditionalTokens.setMarketFactory(factoryAddress);
  await tx.wait();
  console.log("✅ MarketFactory authorized in ConditionalTokens");
  ...
  if (process.env.BASESCAN_API_KEY) {
    ...
    console.log("=".repeat(80));
    console.log(`npx hardhat verify --network ${network.name} ${conditionalTokensAddress} "${deployer.address}"`);
