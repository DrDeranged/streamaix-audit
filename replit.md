...
The system employs a cost-effective strategy for AI services, primarily using GPT-4o-mini for most AI operations (90%+ cost reduction) and reserving GPT-4o only for premium video content analysis (`rebuiltContentProcessor.ts`). Service cycle times for various AI components have been significantly optimized (e.g., AI Meta-Trader from 60min to 6 hours), leading to an estimated monthly OpenAI API cost of $15-25.
