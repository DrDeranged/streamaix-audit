...
### Design System (April 2026 — Task #19 Visual Facelift)
The in-app aesthetic was unified to match the polish of the landing page without changing layout, copy, or features.
- **Tokens** (`client/src/index.css`): dark-mode `--card` / `--popover` lifted to a richer slate, `--border` purple-tinted, `--primary` switched to brand purple `hsl(258, 84%, 62%)`, `--ring` set to brand cyan, `--radius` bumped to `0.625rem`. A new "facelift v1" block adds `--section-*` rhythm vars, `.surface-1` / `.surface-2` / `.surface-3` glass elevations (with light-mode counterparts), `.surface-interactive` hover lift, `.brand-focus-ring`, `.btn-gradient-glow`, `.text-overline` / `.text-display` / `.numeric` utilities, and a global `prefers-reduced-motion` guard.
- **Tailwind** (`tailwind.config.ts`): added `neon` color family (purple, cyan, fuchsia, amber, emerald, rose) and explicit `display` (Orbitron) / `sans` (Inter) `fontFamily`.
- **Themed shadcn primitives**: Button (new `gradient-glow` variant + brand cyan focus ring), Card (default glass + animated hover-glow), Progress (purple→fuchsia→cyan gradient), Tabs (gradient active state + cyan underline glow), Badge (added `success`/`warning`/`info`/`gradient` variants), Slider (purple→cyan gradient + glowing cyan thumb), Switch (gradient checked state).
- **Page sweep**: dashboard, portfolio dashboard, avatar-feed, AvatarFeedWidget, and avatar-leaderboard widgets migrated off ad-hoc `bg-white/5` / `bg-slate-950/40` mixes onto `surface-1` / `surface-interactive` (semantic accent borders preserved). Numeric/PnL/balance cells across the leaderboard and portfolio use the `.numeric` utility for tabular alignment.
- **Reference**: `/style-guide` route renders every primitive, all three glass elevations, the type scale, the neon palette, and a numeric-table example so future screens compose from a single source.
