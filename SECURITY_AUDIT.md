...
- **Policy verification (latest run)**: ✅ **0 errors**, 131 informational warnings. Warnings are now categorized as `global_only_body_validation` — they identify mutations that rely on the **baseline** global `requireJsonObjectBody` validator rather than a tighter per-route Zod schema. Every one of those 131 routes is still body-validated (via the global middleware) and authenticated; the warning is a roadmap for incrementally tightening field-level schemas, not a missing defense. The five required hard policies (mutations require auth, admin paths require admin, AI-heavy mutations require rate limit, financial mutations require both rate limit AND Zod, AI/admin mutations require Zod) are 100% green and will fail CI on regression.
